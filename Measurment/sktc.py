...
# INPUT_FOLDER = r'D:\Automtion\Measurement_Input\test90'
...
def get_top_view_contours(image_path, cl=7, ch=185, ab=33, ac=0, bl=7, debug=False):
...
def process_component(top_view_path, side_view_path, cl=7, ch=185, ab=33, ac=0, bl=7):
    # This call now matches the updated definition above
    top_image, top_contours, top_hierarchy = get_top_view_contours(
        top_view_path, cl, ch, ab, ac, bl, debug=False
    )

    # 1. Get Contours using calibrated values
    top_image, top_contours, top_hierarchy = get_top_view_contours(
        top_view_path, cl, ch, ab, ac, bl, debug=False
    )
    ...
    # 3. Initial filter for position, size, and circularity
    for i, c in enumerate(top_contours):
        if np.array_equal(c, main_contour): continue

        # Ensure it is inside the main disk
        test_point = tuple(c[0][0])
        if main_mask[test_point[1], test_point[0]] != 255: continue
    ...
    # 4. INTENSITY & OVERLAP FILTER: Fixes the "Hole inside a Hole" issue
    # Sort children by area (largest first)
    raw_children = sorted(raw_children, key=cv2.contourArea, reverse=True)
    final_children = []

    for c in raw_children:
        # Get center and radius of the current contour
        (cx, cy), radius = cv2.minEnclosingCircle(c)

        # Check if this contour is inside a larger hole we already accepted
        is_inside_existing = False
        for accepted in final_children:
            (ax, ay), a_radius = cv2.minEnclosingCircle(accepted)
            distance = np.sqrt((cx - ax) ** 2 + (cy - ay) ** 2)
            # If the centers are close, it's a "ghost" inner edge
            if distance < a_radius:
                is_inside_existing = True
                break

        if is_inside_existing:
            print(f"  -> Skipping ghost inner edge at center ({int(cx)}, {int(cy)})")
            continue

        # Intensity check: Real holes show the white paper underneath (high brightness)
        # or a dark void (low brightness). Surface reflections are usually gray (middle).
        h_mask = np.zeros_like(gray)
        cv2.drawContours(h_mask, [c], -1, 255, -1)
        mean_brightness = cv2.mean(gray, mask=h_mask)[0]

        # Filter out anything that isn't distinctly bright or dark
        if mean_brightness > 180 or mean_brightness < 90:
            final_children.append(c)
        else:
            print(f"  -> Skipping reflection based on brightness: {mean_brightness:.1f}")

    # 5. Continue with measurements using final_children
    ...
    if shape_type == "cylinder":
        measurements.update(measure_cylinder(main_contour, final_children, PPM_TOP))
    elif shape_type == "cuboid":
